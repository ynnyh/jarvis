...
async function test() {
  ...
  async function exploreProject(projectId: number, depth: number = 0) {
    ...
    if (visitedProjects.has(projectId)) {
      console.log(`${indent}⏩ 项目 ${projectId} 已访问，跳过`)
      return
    }
    visitedProjects.add(projectId)

    // 获取项目详情
    console.log(`${indent}📁 探索项目 ${projectId}...`)
    let project: any
    try {
      const projectRes = await http.get(`/api.php/v1/projects/${projectId}`)
      project = projectRes.data
      console.log(`${indent}   名称: ${project.name}`)
    } catch (err: any) {
      console.log(`${indent}   ❌ 获取项目详情失败: ${err.response?.status}`)
      return
    }

    // 1. 获取项目下的执行
    console.log(`${indent}   🔍 查找执行...`)
    let execPage = 1
    ...
    while (execPage <= execTotalPages) {
      ...
      if (execPage === 1) {
        ...
        console.log(`${indent}   找到 ${execRes.data.total || executions.length} 个执行`)
      }

      for (const exec of executions) {
        if (exec.id) {
          await exploreExecution(exec.id, depth + 1)
        } else {
          console.log(`${indent}   ⚠️ 执行没有ID:`, JSON.stringify(exec).slice(0, 100))
        }
      }
    ...
    while (childPage <= childTotalPages) {
      ...
      if (childPage === 1) {
        ...
        console.log(`${indent}   找到 ${childRes.data.total || children.length} 个子项目`)
      }

      for (const child of children) {
        if (child.id) {
          await exploreProject(child.id, depth + 1)
        }
      }
  ...
  // 遍历执行下的任务
  async function exploreExecution(executionId: number, depth: number) {
    const indent = '  '.repeat(depth)

    if (visitedExecutions.has(executionId)) {
      console.log(`${indent}⏩ 执行 ${executionId} 已访问，跳过`)
      return
    }
    visitedExecutions.add(executionId)

    console.log(`${indent}⚙️ 探索执行 ${executionId}...`)

    let taskPage = 1
    let taskTotalPages = 1
    let execTotalTasks = 0
    let execMyTasks = 0
    ...
    console.log(`${indent}   任务: ${execTotalTasks}, 我的: ${execMyTasks}`)
  }

  // 从项目 288 开始
  await exploreProject(288)
  ...
  if (allMyTasks.length > 0) {
    ...
    allMyTasks.forEach((t, i) => {
      console.log(`  ${i + 1}. [${t.status}] ${t.taskName} (执行ID: ${t.executionId})`)
    })
  }
}
