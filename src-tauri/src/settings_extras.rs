...
pub async fn pick_directory(title: Option<String>) -> Result<Option<String>, String> {
    ...
    let picked =
        tokio::task::spawn_blocking(move || FileDialog::new().set_title(&title).pick_folder())
            .await
...
pub fn excluded_business_lines_load() -> Result<Vec<String>, String> {
    ...
    let parsed: serde_json::Value =
        serde_json::from_str(&raw).map_err(|e| format!("排除列表解析失败: {}", e))?;
    let list = parsed
        .as_array()
        .map(|arr| {
            arr.iter()
...
pub async fn excluded_business_lines_save(lines: Vec<String>) -> Result<(), String> {
    ...
    crate::util::write_atomic(&excluded_file(), &content)
