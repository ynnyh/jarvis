...
impl ZentaoClient {
    ...
    /// 自动尝试 /work（单数）和 /works（复数）两种端点。
    pub async fn get_task_works(&self, task_id: &str) -> Result<Vec<Value>, String> {
        let token = self.ensure_token().await?;

        // 尝试两种端点：先 /work（单数），404 则 /works（复数）
        let endpoints = [
            format!("api.php/v1/tasks/{}/work", task_id),
            format!("api.php/v1/tasks/{}/works", task_id),
        ];
        ...
        for (i, path) in endpoints.iter().enumerate() {
            ...
            let resp = self
                ...
                .send()
                .await
                .map_err(|e| format!("读任务工作日志失败({}): {}", path, e))?;
            let status = resp.status();
            let text = resp.text().await.unwrap_or_default();

            eprintln!(
                "[zentao] get_task_works({}) 尝试[{}] {} => HTTP {} body_len={} body={}",
                task_id, i, path, status.as_u16(), text.len(),
                crate::util::truncate_chars(&text, 500)
            );

            // 404 尝试下一个端点
            if status.as_u16() == 404 && i == 0 {
                eprintln!("[zentao] get_task_works({}) /work 返回 404，尝试 /works", task_id);
                continue;
            }
            if !status.is_success() {
                return Err(format!(
            ...
            let works = if let Some(arr) = json.as_array() {
                arr.clone()
            } else if let Some(arr) = json.get("work").and_then(|v| v.as_array()) {
                arr.clone()
        ...
        // 两个端点都 404
        eprintln!("[zentao] get_task_works({}) 两种端点都 404", task_id);
        Ok(Vec::new())
    }
