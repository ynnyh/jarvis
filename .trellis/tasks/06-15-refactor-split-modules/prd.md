# 架构拆分:llm.rs / App.vue 两大文件模块化

> 阶段一(内功期)任务 3/3,也是最大最危险的任务。
> 前置已就绪:可观测性(tracing 日志)+ 测试网(worklog/settings/核心模块测试 + CI)。
> 目标:把"巨型文件"拆成模块,降低单文件复杂度,为后续团队期/开源期的可维护性打基础。
>
> ⚠️ 范围变更(06-16):语音功能已整体下线(见 06-16-remove-voice-input),voice.rs 已删除,
> 原 Batch 2(voice.rs 拆分)取消。本任务现只剩 llm.rs(Batch 1)+ App.vue(Batch 2)两批。
...
两个文件已到"再加功能就失控"的临界点:
- **llm.rs(1548 行)**:chat / responses / anthropic 三协议实现 + 公共入口,三套协议的转换函数交错。
...
### 2.2 App.vue → 抽 useWindowOrchestration composable
...
**Batch 2:App.vue**(风险最高,前端无类型检查网,逻辑改动微妙)
...
| # | 条件 | 验证方式 |
|---|------|---------|
| 1 | llm.rs 拆成 llm/{mod,chat,responses,anthropic}.rs,单文件 < 500 行 | wc -l |
| 2 | App.vue < 500 行,编排逻辑在 useWindowOrchestration.ts | wc -l |
| 3 | 业务行为不变:现有测试全部通过 | cargo test(CI)|
| 4 | cargo check + clippy 通过(尽量 -D warnings) | 本地 + CI |
| 5 | check:text + vue-tsc 通过 | 本地 |
| 6 | 无业务逻辑改动(纯 move) | code review diff |
...
| 两批工作量大 | 分两批独立 commit,每批可单独回滚;允许跨多个 session |
